...
namespace Test.Classifier
{
    public class DeepNetworkTest : ClassifierTest
    {
        ...
        public void TestTrain()
        {
            ...
            Assert.AreEqual(66.67, 100 * deepNetwork.Test(iris.GetInstanceList()).GetErrorRate(), 0.01);
            ...
            Assert.AreEqual(28.41, 100 * deepNetwork.Test(bupa.GetInstanceList()).GetErrorRate(), 0.01);
            deepNetworkParameter = new DeepNetworkParameter(1, 0.01, 0.99, 0.2, 100, new List<int>
                {20});
            deepNetwork.Train(dermatology.GetInstanceList(), deepNetworkParameter);
            Assert.AreEqual(3.82, 100 * deepNetwork.Test(dermatology.GetInstanceList()).GetErrorRate(), 0.01);
        }
    }
}